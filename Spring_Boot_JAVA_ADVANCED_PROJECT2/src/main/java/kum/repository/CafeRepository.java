...
public interface CafeRepository extends JpaNameRepository<Cafe> {
	...
	@Query("SELECT new kum.model.view.CafeView(c.id, c.rate, c.name, c.photoUrl, c.version, c.address, c.fullDescription, c.type, c.phone, c.web, open.time, close.time) FROM Cafe c LEFT JOIN c.open open LEFT JOIN c.close close")
